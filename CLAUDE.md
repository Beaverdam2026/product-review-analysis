...
## Working Style
When a tool call (web fetch, search, file read) returns empty, truncated, or error output, explicitly say so before continuing, and do not describe or summarize content you did not actually receive. When writing terminal commands and code, explain the basic logic of what the command is doing. If there is an advanced or uncommon aspect of the command, explain in greater detail.

## Project
SQL-focused portfolio project analyzing Amazon reviews. SQL is the focus (aiming to strengthen window functions and CTEs specifically); Python is for visualizations only. Keep .sql files separate from Python as much as is reasonable.
...
## Environment
Fedora laptop. Python 3.14, SQLite for the database. Using Claude Code with Opus for scaffolding/syntax, writing core analytical queries by hand.
